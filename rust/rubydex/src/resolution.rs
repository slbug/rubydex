...
impl<'a> Resolver<'a> {
        ...
        F: FnOnce(String) -> Declaration,
    {
        ...
        if matches!(
            ...
            // A newly-created method can satisfy unresolved visibility calls or replace an inherited
            // `module_function` copy on descendant namespaces. Resolve those dependents immediately
            // so diagnostics and generated copies converge in the same resolution cycle.
            let visibility_ids = self
                .graph
                .invalidate_method_visibility_dependents_for_member_change(owner_id, str_id);
            if !visibility_ids.is_empty() {
                self.resolve_method_visibilities(visibility_ids);
            }
        }
    }
